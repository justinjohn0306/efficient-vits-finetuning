...
def run(rank, n_gpus, hps):
  with wandb.init(project=f"vits_finetuning_{hps.data.name}", config=hps, resume=hps.train.wandb_resume):
    ...
    try:
      _, _, _, epoch_str = utils.load_lora_checkpoint(utils.latest_checkpoint_path(hps.model_dir, "G_*.pth"), net_g, optim_g)
    ...
    if rank==0:
      ...
      if global_step % hps.train.eval_interval == 0:
        ...
        with torch.no_grad():
            ...
            audio = net_g.infer(x_tst.cuda(0), x_tst_lengths.cuda(0), noise_scale=.667, noise_scale_w=0.8, length_scale=1)[0][0,0].data.float().cpu().numpy()
        ...
        utils.save_lora_checkpoint(net_g, optim_g, hps.train.learning_rate, epoch, os.path.join(hps.model_dir, "G_{}.pth".format(global_step)))

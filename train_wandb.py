...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
  ...
  for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths) in enumerate(train_loader):
    ...
    if rank==0:
      if global_step % hps.train.log_interval == 0:
        ...
        scalar_dict.update({"loss/g/fm": loss_fm, "loss/g/mel": loss_mel, "loss/g/dur": loss_dur, "loss/g/kl": loss_kl, "loss_disc":loss_disc, "loss_gen":loss_gen})
  ...
  if int(epoch) % hps.train.infer_interval == 0:
    stn_tst = get_text("My name is Optimus Prime, and this is a test of my voice generated by VITS text to speech.", hps)
    with torch.no_grad():
        x_tst = stn_tst.unsqueeze(0)
        x_tst_lengths = torch.LongTensor([stn_tst.size(0)])
